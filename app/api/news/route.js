...
function clean(s=""){return s.replace(/<!\[CDATA\[/g,"").replace(/\]\]>/g,"").replace(/<[^>]+>/g,"").trim()}
...
function parse(xml,source){
 ...
 return blocks.slice(0,6).map(b=>{
  ...
  return title&&link?{section:"Latest AI",tag:"SOURCE FEED",title:clean(title),dek:"Fresh update from "+source+". Open the source for the full announcement.",source,url:clean(link),date}:null;
 }).filter(Boolean);
}
...
export async function GET(){
 ...
 await Promise.all(feeds.map(async ([source,url])=>{
  try{
   ...
   if(r.ok) all.push(...parse(await r.text(),source));
  }catch{}
 }));
 return Response.json(
  {items:all.slice(0,30),generatedAt:new Date().toISOString()},
